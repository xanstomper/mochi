...
func TestTracker_BuiltinSkillTracking(t *testing.T) {
	...
	// Simulate active skills including a builtin skill (MOCHI-config).
	activeSkills := []*Skill{
		{Name: "MOCHI-config", Description: "MOCHI config", Builtin: true},
		{Name: "go-doc", Description: "Go docs", Builtin: false},
	}
	tracker := NewTracker(activeSkills)

	// Initially not loaded.
	require.False(t, tracker.IsLoaded("MOCHI-config"))
	require.False(t, tracker.IsLoaded("go-doc"))

	// Mark builtin skill as loaded (simulating read via MOCHI://...).
	tracker.MarkLoaded("MOCHI-config")
	require.True(t, tracker.IsLoaded("MOCHI-config"))
