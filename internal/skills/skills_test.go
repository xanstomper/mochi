...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "MOCHI://skills/builtin-skill/SKILL.md", Builtin: true},
		{Name: "user-skill", Description: "A user skill.", SkillFilePath: "/home/user/.config/MOCHI/skills/user-skill/SKILL.md"},
	}
	xml := ToPromptXML(skills)
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "MOCHI-config" {
			found = true
			require.True(t, strings.HasPrefix(s.SkillFilePath, BuiltinPrefix))
			require.True(t, strings.HasPrefix(s.Path, BuiltinPrefix))
			require.Equal(t, "MOCHI://skills/MOCHI-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "MOCHI://skills/MOCHI-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "MOCHI-config builtin skill not found")

	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "jq" {
			foundJQ = true
			require.Equal(t, "MOCHI://skills/jq/SKILL.md", s.SkillFilePath)
			require.Equal(t, "MOCHI://skills/jq", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	...
	var foundHooks bool
	for _, s := range discovered {
		if s.Name == "MOCHI-hooks" {
			foundHooks = true
			require.Equal(t, "MOCHI://skills/MOCHI-hooks/SKILL.md", s.SkillFilePath)
			require.Equal(t, "MOCHI://skills/MOCHI-hooks", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, foundHooks, "MOCHI-hooks builtin skill not found")
}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "MOCHI-config", Path: "MOCHI://skills/MOCHI-config"}, {Name: "MOCHI-config", Path: "/user/MOCHI-config"}},
			wantLen:  1,
			wantName: "MOCHI-config",
			wantPath: "/user/MOCHI-config",
		},
		{
			name:    "empty",
			input:   nil,
