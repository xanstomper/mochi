---
name: MOCHI-hooks
description: Use when the user wants to add, write, debug, or configure a MOCHI hook — gating or blocking tool calls, approving or rewriting tool input before execution, injecting context into tool results, or troubleshooting hook behavior in MOCHI.json.
---

# MOCHI Hooks

Hooks are user-defined commands in `MOCHI.json` that fire at specific points
...
3. Add the entry under `hooks.PreToolUse` in `MOCHI.json` with the right matcher.
...
- Non-zero exit codes other than 2/49 are logged but don't block — check MOCHI logs.
...
MOCHI also accepts Claude Code's `hookSpecificOutput` envelope. One intentional
divergence: MOCHI treats `updated_input` as shallow-merge, Claude Code replaces.
