...
func TestMOCHIIgnore(t *testing.T) {
	tempDir := t.TempDir()
	t.Chdir(tempDir)
	...
	// Create a .MOCHIignore file that ignores .log files
	require.NoError(t, os.WriteFile(".MOCHIignore", []byte("*.log\n"), 0o644))
...
func TestShouldExcludeFile(t *testing.T) {
	...
	// Create .MOCHIignore file
	MOCHIignoreContent := "custom_ignored/\n"
	if err := os.WriteFile(filepath.Join(tempDir, ".MOCHIignore"), []byte(MOCHIignoreContent), 0o644); err != nil {
		t.Fatalf("Failed to create .MOCHIignore: %v", err)
	}
	...
	require.True(t, ShouldExcludeFile(tempDir, customIgnored), "Expected custom_ignored to be ignored by .MOCHIignore")
...
func TestShouldExcludeFileHierarchical(t *testing.T) {
	...
	// Create .MOCHIignore in subdir that ignores normal_nested
	subMOCHIignore := "normal_nested/\n"
	if err := os.WriteFile(filepath.Join(subDir, ".MOCHIignore"), []byte(subMOCHIignore), 0o644); err != nil {
		t.Fatalf("Failed to create subdir .MOCHIignore: %v", err)
	}

	// Test hierarchical ignore behavior - this should work because the .MOCHIignore is in the parent directory
	require.True(t, ShouldExcludeFile(tempDir, nestedNormal), "Expected normal_nested to be ignored by subdir .MOCHIignore")
