...
// MOCHIGlobalIgnorePatterns returns patterns from the user's
// ~/.config/MOCHI/ignore file.
var MOCHIGlobalIgnorePatterns = sync.OnceValue(func() []gitignore.Pattern {
	name := filepath.Join(home.Config(), "MOCHI", "ignore")
	bts, err := os.ReadFile(name)
	if err != nil {
		if !os.IsNotExist(err) {
			slog.Debug("Failed to read MOCHI global ignore file", "path", name, "error", err)
		}
		return nil
	}
...
type directoryLister struct {
	// dirPatterns caches parsed patterns from .gitignore/.MOCHIignore for each directory.
...
// .gitignore and .MOCHIignore files. Results are cached.
func (dl *directoryLister) getDirPatterns(dir string) []gitignore.Pattern {
	return dl.dirPatterns.GetOrSet(dir, func() []gitignore.Pattern {
		...
		for _, ignoreFile := range []string{".gitignore", ".MOCHIignore"} {
...
func (dl *directoryLister) getCombinedMatcher(dir string) gitignore.Matcher {
	return dl.combinedMatchers.GetOrSet(dir, func() gitignore.Matcher {
		...
		// Add global ignore patterns (git core.excludesFile + MOCHI global ignore).
		allPatterns = append(allPatterns, gitGlobalIgnorePatterns()...)
		allPatterns = append(allPatterns, MOCHIGlobalIgnorePatterns()...)
