...
import (
	...
	"github.com/mochi/mochi/internal/projects"
	...
	ui "github.com/mochi/mochi/internal/ui/model"
...
func init() {
	...
	rootCmd.PersistentFlags().StringP("data-dir", "D", "", "Custom mochi data directory")
	rootCmd.PersistentFlags().BoolP("debug", "d", false, "Debug")
	rootCmd.PersistentFlags().StringVarP(&clientHost, "host", "H", server.DefaultHost(), "Connect to a specific mochi server host (for advanced users)")
...
cat README.md | mochi run "make this more glamorous" > GLAMOROUS_README.md

# Run with debug logging in a specific directory
mochi --debug --cwd /path/to/project

# Run in yolo mode (auto-accept all permissions; use with care)
mochi --yolo

# Run with custom data directory
mochi --data-dir /path/to/custom/.mochi

# Continue a previous session
mochi --session {session-id}
...
`,
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if _, err := program.Run(); err != nil {
			...
			return errors.New("Mochi crashed. If metrics are enabled, we were notified about it. If you'd like to report it, please copy the stacktrace above and open an issue at https://github.com/mochi/mochi/issues/new?template=bug.yml") //nolint:staticcheck
		}
		return nil
	},
}
...
// enabled via the MOCHI_CLIENT_SERVER environment variable.
func useClientServer() bool {
	v, _ := strconv.ParseBool(os.Getenv("MOCHI_CLIENT_SERVER"))
	return v
}
...
// MOCHI_CLIENT_SERVER=1, it connects to a server process and returns a
...
func setupLocalWorkspace(cmd *cobra.Command) (workspace.Workspace, func(), error) {
	...
	logFile := filepath.Join(cfg.Options.DataDirectory, "logs", "MOCHI.log")
...
func connectToServer(cmd *cobra.Command) (*client.Client, *proto.Workspace, func(), error) {
	...
	if ws.Config != nil {
		logFile := filepath.Join(ws.Config.Options.DataDirectory, "logs", "MOCHI.log")
		MOCHIlog.Setup(logFile, debug)
	}
	...
	case "unix", "npipe":
		...
		if needsStart {
			if err := spawnAndWaitReady(cmd, hostURL); err != nil {
				return fmt.Errorf("failed to initialize MOCHI server: %v", err)
			}
			return nil
		}
...
// $XDG_CACHE_HOME/MOCHI/server-<safeHost>/start.lock.
//
...
// Overridable via MOCHI_SERVER_READY_TIMEOUT (parsed as a Go duration).
func serverReadyTimeout() time.Duration {
	const def = 10 * time.Second
	v := os.Getenv("MOCHI_SERVER_READY_TIMEOUT")
	if v == "" {
		return def
	}
...
func startDetachedServer(cmd *cobra.Command, hostURL *url.URL) error {
	...
	if err := c.Start(); err != nil {
		return fmt.Errorf("failed to start MOCHI server: %v", err)
	}

	if err := c.Process.Release(); err != nil {
		return fmt.Errorf("failed to detach MOCHI server process: %v", err)
	}
...
func shouldEnableMetrics(cfg *config.Config) bool {
	if v, _ := strconv.ParseBool(os.Getenv("MOCHI_DISABLE_METRICS")); v {
		return false
	}
...
func createDotMOCHIDir(dir string) error {
