...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["MOCHI_EVENT"])
	require.Equal(t, "bash", envMap["MOCHI_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["MOCHI_SESSION_ID"])
	require.Equal(t, "/work", envMap["MOCHI_CWD"])
	require.Equal(t, "/project", envMap["MOCHI_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["MOCHI_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["MOCHI_TOOL_INPUT_FILE_PATH"])

	// Shared MOCHI markers must be present so hook-authored scripts can
	// detect they're running under MOCHI the same way bash-tool-invoked
	// scripts can.
	require.Equal(t, "1", envMap["MOCHI"])
	require.Equal(t, "MOCHI", envMap["AGENT"])
	require.Equal(t, "MOCHI", envMap["AI_AGENT"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$MOCHI_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("MOCHI format still works", func(t *testing.T) {
		t.Parallel()
