...
func TestLookupConfigs_BoundedByProject(t *testing.T) {
	...
	globalDir := t.TempDir()
	t.Setenv("MOCHI_GLOBAL_CONFIG", globalDir)
	t.Setenv("MOCHI_GLOBAL_DATA", globalDir)

	t.Run("does not pick up MOCHI.json above non-git project", func(t *testing.T) {
		parent := t.TempDir()

		// MOCHI.json above the project must not be adopted.
		require.NoError(t, os.WriteFile(
			filepath.Join(parent, "MOCHI.json"),
			[]byte(`{}`),
			0o644,
		))
		...
		for _, p := range got {
			require.NotEqual(t, filepath.Join(parent, "MOCHI.json"), p)
		}
	})

	t.Run("does not climb out of git worktree to find MOCHI.json", func(t *testing.T) {
		...
		strayEval, err := filepath.EvalSymlinks(filepath.Join(parent, "MOCHI.json"))
		require.NoError(t, err)
		for _, p := range got {
			...
			require.NotEqual(t, strayEval, pEval, "must not adopt parent MOCHI.json")
		}
	})

	t.Run("picks up MOCHI.json inside the project", func(t *testing.T) {
		project := t.TempDir()
		local := filepath.Join(project, "MOCHI.json")
		...
		require.True(t, foundLocal, "expected project MOCHI.json to be in lookup result: %v", got)
	})
...
func TestConfig_setDefaults(t *testing.T) {
	t.Run("sets default data directory", func(t *testing.T) {
		...
		require.Equal(t, filepath.Join(workingDir, defaultDataDirectory), cfg.Options.DataDirectory)
	...
	t.Run("does not adopt .MOCHI from a parent project", func(t *testing.T) {
		parent := t.TempDir()

		// .MOCHI in the parent: it should not be reused by the child
	...
	t.Run("does not climb out of git worktree to find .MOCHI", func(t *testing.T) {
		if _, err := exec.LookPath("git"); err != nil {
			t.Skip("git not available")
		}
		...
		// Stray .MOCHI above the worktree root.
		require.NoError(t, os.Mkdir(filepath.Join(parent, defaultDataDirectory), 0o755))
		...
		require.NoError(t, err)
		require.NotEqual(t, strayEval, gotEval, "must not adopt parent .MOCHI")
...
func TestConfig_setupAgentsWithDisabledTools(t *testing.T) {
	...
	assert.Equal(t, []string{"agent", "bash", "MOCHI_info", "MOCHI_logs", "job_output", "job_kill", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "glob", "ls", "sourcegraph", "todos", "view", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_setupAgentsWithEveryReadOnlyToolDisabled(t *testing.T) {
	...
	coderAgent, ok := cfg.Agents[AgentCoder]
	require.True(t, ok)
	assert.Equal(t, []string{"agent", "bash", "MOCHI_info", "MOCHI_logs", "job_output", "job_kill", "download", "edit", "multiedit", "lsp_diagnostics", "lsp_references", "lsp_restart", "fetch", "agentic_fetch", "todos", "write", "list_mcp_resources", "read_mcp_resource"}, coderAgent.AllowedTools)
...
func TestConfig_setDefaultsDisableDefaultProvidersEnvVar(t *testing.T) {
	t.Run("sets option from environment variable", func(t *testing.T) {
		t.Setenv("MOCHI_DISABLE_DEFAULT_PROVIDERS", "true")
...
func TestConfig_configureSelectedModels(t *testing.T) {
	t.Run("reload mode should not persist fallback defaults", func(t *testing.T) {
		dir := t.TempDir()
		globalPath := filepath.Join(dir, "MOCHI.json")
