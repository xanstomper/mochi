...
func PushPopMOCHIEnv() func() {
	var found []string
	for _, ev := range os.Environ() {
		if strings.HasPrefix(ev, "MOCHI_") {
			pair := strings.SplitN(ev, "=", 2)
			if len(pair) != 2 {
				continue
			}
			found = append(found, strings.TrimPrefix(pair[0], "MOCHI_"))
		}
	}
	...
	for _, ev := range found {
		os.Setenv(ev, os.Getenv("MOCHI_"+ev))
	}
...
func (c *Config) setDefaults(workingDir, dataDir string) {
	...
	if str, ok := os.LookupEnv("MOCHI_DISABLE_PROVIDER_AUTO_UPDATE"); ok {
		c.Options.DisableProviderAutoUpdate, _ = strconv.ParseBool(str)
	}

	if str, ok := os.LookupEnv("MOCHI_DISABLE_DEFAULT_PROVIDERS"); ok {
...
// so an unrelated MOCHI.json placed above the project is never picked
...
func lookupConfigs(cwd string) []string {
	...
	configNames := []string{appName + ".json", "." + appName + ".json", "MOCHI.json", ".MOCHI.json"}
...
func GlobalConfig() string {
	if MOCHIGlobal := os.Getenv("MOCHI_GLOBAL_CONFIG"); MOCHIGlobal != "" {
		return filepath.Join(MOCHIGlobal, fmt.Sprintf("%s.json", appName))
	}
...
func GlobalCacheDir() string {
	if MOCHICache := os.Getenv("MOCHI_CACHE_DIR"); MOCHICache != "" {
		return MOCHICache
	}
...
func GlobalConfigData() string {
	if MOCHIData := os.Getenv("MOCHI_GLOBAL_DATA"); MOCHIData != "" {
		return filepath.Join(MOCHIData, fmt.Sprintf("%s.json", appName))
	}
	...
	// for windows, it should be in `%LOCALAPPDATA%/MOCHI/`
	// for linux and macOS, it should be in `$HOME/.local/share/MOCHI/`
	if runtime.GOOS == "windows" {
		localAppData := cmp.Or(
			os.Getenv("LOCALAPPDATA"),
			filepath.Join(os.Getenv("USERPROFILE"), "AppData", "Local"),
...
// fallback keeps MOCHI from silently adopting state files placed above
// the current project.
...
func GlobalSkillsDirs() []string {
	if MOCHISkills := os.Getenv("MOCHI_SKILLS_DIR"); MOCHISkills != "" {
		return []string{MOCHISkills}
	}
	...
	// On Windows, also load from app data on top of `$HOME/.config/MOCHI`.
...
// ProjectSkillsDir returns the default project directories for which MOCHI
// will look for skills.
func ProjectSkillsDir(workingDir string) []string {
	return []string{
		filepath.Join(workingDir, ".agents/skills"),
		filepath.Join(workingDir, ".MOCHI/skills"),
