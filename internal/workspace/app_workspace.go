...
// AgentCoordinator returns the underlying app's coordinator so
// the swarm runtime can dispatch work to live LLM sessions.
// Returns nil if the coordinator hasn't been built yet.
func (w *AppWorkspace) AgentCoordinator() agent.Coordinator {
	if w.app == nil {
		return nil
	}
	return w.app.AgentCoordinator
}
