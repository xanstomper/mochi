...
func quickStyle(o quickStyleOpts) Styles {
	...
	// Editor
	s.Editor.PromptNormalFocused = lipgloss.NewStyle().Foreground(o.primary).SetString("> ")
	s.Editor.PromptNormalBlurred = s.Editor.PromptNormalFocused.Foreground(o.fgMoreSubtle)
	s.Editor.PromptYoloIconFocused = lipgloss.NewStyle().MarginRight(1).Foreground(o.onPrimary).Background(o.busy).Bold(true).SetString(" ! ")
	...
	s.Messages.UserBlurred = s.Messages.NoContent.PaddingLeft(1).BorderLeft(true).
		BorderForeground(o.separator).BorderStyle(lipgloss.NormalBorder())
	...
	s.Messages.AssistantFocused = s.Messages.NoContent.PaddingLeft(1).BorderLeft(true).
		BorderForeground(o.separator).BorderStyle(messageFocussedBorder)
	...
	// Dialog styles
	s.Dialog.Title = base.Padding(0, 1).Foreground(o.fgBase)
	s.Dialog.TitleText = base.Foreground(o.fgBase)
	s.Dialog.TitleError = base.Foreground(o.destructive)
	s.Dialog.TitleAccent = base.Foreground(o.primary).Bold(true)
	s.Dialog.TitleLineBase = lipgloss.NewStyle()
	s.Dialog.TitleGradFromColor = o.separator
	s.Dialog.TitleGradToColor = o.primary
	...
	s.Dialog.Quit.Frame = lipgloss.NewStyle().BorderForeground(o.separator).Border(lipgloss.NormalBorder()).Padding(1, 2)
	s.Dialog.View = base.Border(lipgloss.NormalBorder()).BorderForeground(o.separator)
	...
	s.Status.SuccessIndicator = base.Foreground(o.onPrimary).Background(o.success).Padding(0, 1).Bold(true).SetString("OK")
	...
	s.Status.WarnIndicator = s.Status.SuccessIndicator.Foreground(o.onPrimary).Background(o.warning).SetString("WARN")
	s.Status.ErrorIndicator = s.Status.SuccessIndicator.Foreground(o.onPrimary).Background(o.destructive).SetString("ERR")
