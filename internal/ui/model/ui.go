...
import (
	...
	agenttools "github.com/mochi/mochi/internal/agent/tools"
	...
	"github.com/mochi/mochi/internal/stringext"
	...
	"github.com/mochi/mochi/internal/ui/completions"
	"github.com/mochi/mochi/internal/ui/dialog"
	fimage "github.com/mochi/mochi/internal/ui/image"
	"github.com/mochi/mochi/internal/ui/logo"
	"github.com/mochi/mochi/internal/ui/notification"
	"github.com/mochi/mochi/internal/ui/styles"
	"github.com/mochi/mochi/internal/ui/util"
...
type UI struct {
	...
	// swarmEnabled is the in-memory "swarm mode" toggle. When
	// on, sendMessage routes the prompt through a heavier
	// model with a swarm-rules prefix so the agent behaves
	// like a multi-agent orchestrator in a single session.
	// Closing the TUI resets it. No new dialog, no new
	// runtime — the existing MOCHI coordinator does the work.
	swarmEnabled bool
	// swarmPriorLarge remembers the LARGE model the user had
	// selected before toggling swarm on, so revertSwarmMode
	// can put it back. The agent always reads from the LARGE
	// slot, so this is the only knob that actually moves the
	// model that runs the conversation.
	swarmPriorLarge config.SelectedModel
...
func (m *UI) Update(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case app.UpdateAvailableMsg:
		text := fmt.Sprintf("MOCHI update available: v%s → v%s.", msg.CurrentVersion, msg.LatestVersion)
		if msg.IsDevelopment {
			text = fmt.Sprintf("This is a development version of MOCHI. The latest version is v%s.", msg.LatestVersion)
		}
		ttl := 10 * time.Second
...
func (m *UI) handleDialogMsg(msg tea.Msg) tea.Cmd {
	...
	case dialog.ActionQuit:
		// If swarm mode is on, restore the user's original LARGE
		// model before quitting so the next session starts with
		// their normal model, not the swarm model.
		if m.swarmEnabled {
			cmds = append(cmds, m.revertSwarmMode())
		}
	...
	case dialog.ActionRunCustomCommand:
		if len(msg.Arguments) > 0 && msg.Args == nil {
			// Prevent opening the same arguments dialog repeatedly (guards against loops)
			newArgsDialog := dialog.NewArguments(
				m.com,
			...
			if m.dialog.ContainsDialog(newArgsDialog.ID()) {
				break
			}
			m.dialog.CloseFrontDialog()
			m.dialog.OpenDialog(newArgsDialog)
			break
		}
		content := msg.Content
		if msg.Args != nil {
	...
	case dialog.ActionToggleSwarmMode:
		...
		m.refreshEditorPlaceholder()
		if m.swarmEnabled {
			cmds = append(cmds, m.applySwarmMode())
			cmds = append(cmds, util.CmdHandler(util.NewInfoMsg("Swarm mode: ON — multi-agent rules active")))
		} else {
			cmds = append(cmds, m.revertSwarmMode())
			cmds = append(cmds, util.CmdHandler(util.NewInfoMsg("Swarm mode: OFF")))
		}
	default:
...
func (m *UI) Draw(scr uv.Screen, area uv.Rectangle) *tea.Cursor {
	...
	if os.Getenv("MOCHI_UI_DEBUG") == "true" {
...
func (m *UI) View() tea.View {
	...
	v.WindowTitle = "MOCHI " + home.Short(m.com.Workspace.WorkingDir())
...
// refreshEditorPlaceholder updates the textarea placeholder
// text to reflect the current state. Priority is:
//  1. swarm mode + busy  → "🐝 Swarm running..."
//  2. yolo mode          → "Yolo mode!"
//  3. swarm mode         → "🐝 Swarm mode — multi-agent active"
//  4. busy               → working placeholder
//  5. idle               → ready placeholder
func (m *UI) refreshEditorPlaceholder() {
	switch {
	case m.swarmEnabled && m.isAgentBusy():
		m.textarea.Placeholder = "🐝 Swarm running..."
	case m.com.Workspace.PermissionSkipRequests():
		m.textarea.Placeholder = "Yolo mode!"
	case m.swarmEnabled:
		m.textarea.Placeholder = "🐝 Swarm mode — multi-agent active"
	case m.isAgentBusy():
...
// When swarm mode is on, the message is wrapped with a swarm-rules
// preamble so the agent behaves like a multi-agent orchestrator
// in the same session — no separate runtime, no dialog, the
// user just types and the model switches behaviour.
func (m *UI) sendMessage(content string, attachments ...message.Attachment) tea.Cmd {
	...
	if m.swarmEnabled {
		content = swarmRulesPreamble + content
	}
...
// swarmRulesPreamble is prepended to the user message when
// swarm mode is on. It tells the model to behave like a
// multi-agent orchestrator: decompose the task, plan, dispatch,
// integrate, all within the single session. This is cheaper
// than spinning up a separate runtime and keeps the chat
// experience unchanged.
const swarmRulesPreamble = `[SWARM MODE ACTIVE — auto-configured by the user]
You are operating as a multi-agent orchestrator. Before answering, internally:
1. Decompose the user's request into 2-6 independent subtasks (planning, research, coding, review, validation, integration).
2. Tackle each subtask in turn, using parallel tool calls where the subtasks are independent.
3. Validate your work: re-read changed files, run targeted tests when cheap, and fix issues before reporting done.
4. Report a concise summary at the end with: files changed, tests run, token cost.

Rules in swarm mode:
- Use diff-only edits (edit/multiedit). Avoid whole-file rewrites.
- Skip ` + "`" + `go test ./...` + "`" + ` / ` + "`" + `npm test` + "`" + ` unless the user asked for full validation; prefer targeted tests on the files you changed.
- Be brief. Output the smallest amount of text that proves the work is done.
- If a subtask is too large, complete the most important parts and list the rest as follow-ups.
- Do not ask clarifying questions; make the most reasonable assumption and proceed.
...
// applySwarmMode configures the active agent for swarm-style
// behaviour: switch the LARGE model slot to the user's configured
// "swarm" model (if any), and stash the original selection so
// revertSwarmMode can restore it. The agent always reads from
// the LARGE slot, so this is the only knob that actually moves
// the model. If the user hasn't configured a "swarm" slot, we
// just leave the LARGE slot alone — the rules injection is the
// primary mechanism, and the user can pick a heavy LARGE
// model in their config.
func (m *UI) applySwarmMode() tea.Cmd {
	cfg := m.com.Config()
	if cfg == nil {
		return nil
	}
	heavy, ok := cfg.Models[config.SelectedModelTypeLarge]
	if !ok {
		slog.Warn("Swarm mode: no large model configured")
		return nil
	}
	// Stash the current LARGE model so revert can restore it.
	m.swarmPriorLarge = heavy
	// If the user has a swarm-preferred model in their config,
	// swap the LARGE slot to it. Otherwise the LARGE slot
	// already does the job and we just enable rules.
	swarmSlot, hasSwarm := cfg.Models[swarmModelSlot]
	if !hasSwarm || sameSelectedModel(swarmSlot, heavy) {
		// Nothing to swap; rules injection does the work.
		return nil
	}
	if err := m.com.Workspace.UpdatePreferredModel(config.ScopeGlobal, config.SelectedModelTypeLarge, swarmSlot); err != nil {
		slog.Warn("Swarm mode: failed to swap large slot", "error", err)
		return nil
	}
	// Push the swap into the in-memory config so subsequent
	// UpdateAgentModel calls see the new value.
	cfg.Models[config.SelectedModelTypeLarge] = swarmSlot
	return func() tea.Msg {
...
// revertSwarmMode restores the LARGE model the user had selected
// before swarm mode was turned on.
func (m *UI) revertSwarmMode() tea.Cmd {
	if m.swarmPriorLarge.Provider == "" && m.swarmPriorLarge.Model == "" {
		return nil
	}
	cfg := m.com.Config()
	if cfg == nil {
		m.swarmPriorLarge = config.SelectedModel{}
		return nil
	}
	prior := m.swarmPriorLarge
	m.swarmPriorLarge = config.SelectedModel{}
	if err := m.com.Workspace.UpdatePreferredModel(config.ScopeGlobal, config.SelectedModelTypeLarge, prior); err != nil {
		slog.Warn("Swarm mode: failed to restore large slot", "error", err)
		return nil
	}
	cfg.Models[config.SelectedModelTypeLarge] = prior
	return func() tea.Msg {
		_ = m.com.Workspace.UpdateAgentModel(context.Background())
		return nil
	}
...
// swarmModelSlot is an out-of-band model slot the user can set
// in their config to point swarm mode at a heavier model than
// the regular LARGE slot. Optional; if absent, swarm mode runs
// on the same LARGE model with rules injection only.
const swarmModelSlot config.SelectedModelType = "swarm"

// sameSelectedModel compares two SelectedModel values by the
// fields that identify a unique model assignment. We can't use
// == directly because SelectedModel contains a map field.
func sameSelectedModel(a, b config.SelectedModel) bool {
...
// renderLogo renders the MOCHI logo with the given styles and dimensions.
