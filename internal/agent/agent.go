// Package agent is the core orchestration layer for MOCHI AI agents.
//
...
var userAgent = fmt.Sprintf("Charm-MOCHI/%s (https://charm.land/MOCHI)", version.Version)
...
type SessionAgentCall struct {
	...
	// reliable completion contract (e.g. `MOCHI run` against a
	...
	// event, so non-interactive clients (e.g. `MOCHI run`) don't
...
func (a *sessionAgent) Run(ctx context.Context, call SessionAgentCall) (result *fantasy.AgentResult, retErr error) {
	...
	if err != nil {
		...
		if isCancelErr {
			...
			currentAssistant.AddFinish(message.FinishReasonError, "Unauthorized", `Please re-authenticate with Hyper. You can also run "MOCHI auth" to re-authenticate.`)
...
func (a *sessionAgent) getCacheControlOptions() fantasy.ProviderOptions {
	if t, _ := strconv.ParseBool(os.Getenv("MOCHI_DISABLE_ANTHROPIC_CACHE")); t {
