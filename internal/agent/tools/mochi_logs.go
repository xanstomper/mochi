...
const MOCHILogsToolName = "MOCHI_logs"

//go:embed MOCHI_logs.md.tpl
var MOCHILogsDescriptionTmpl []byte

var MOCHILogsDescriptionTpl = template.Must(
	template.New("MOCHILogsDescription").
		Parse(string(MOCHILogsDescriptionTmpl)),
)

type MOCHILogsDescriptionData struct {
	DefaultLines int
	MaxLines     int
}

func MOCHILogsDescription() string {
	return renderTemplate(MOCHILogsDescriptionTpl, MOCHILogsDescriptionData{
...
type MOCHILogsParams struct {
	Lines int `json:"lines,omitempty" description:"Number of recent log entries to return (default 50, max 100)"`
}

func NewMOCHILogsTool(logFile string) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		MOCHILogsToolName,
		MOCHILogsDescription(),
		func(ctx context.Context, params MOCHILogsParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			result := runMOCHILogs(logFile, params)
			return fantasy.NewTextResponse(result), nil
		},
	)
}

// runMOCHILogs reads and formats the last N log entries from the given file.
func runMOCHILogs(logFile string, params MOCHILogsParams) string {
