...
// MOCHIEnvMarkers returns a fresh slice of the environment variables that
// MOCHI unconditionally sets on every shell it spawns — both the interactive
...
func NewShell(opts *Options) *Shell {
	...
	// Allow tools to detect execution by MOCHI.
	env = append(env, MOCHIEnvMarkers()...)
