...
import (
	...
	"github.com/mochi/mochi/internal/update"
...
type App struct {
	...
	// subscribers (notably `MOCHI run` in client/server mode) can
...
func New(ctx context.Context, conn *sql.DB, store *config.ConfigStore, skillsMgr *skills.Manager) (*App, error) {
	...
	// Pre-warm the active LLM provider's HTTP connection pool
	// in the background. The first real prompt skips the
	// TLS+TCP handshake when the pool is already warm.
	go app.warmupProviderConnections(ctx)
...
// for `MOCHI run`. A lossy fan-in here can drop the only terminal
...
// warmupProviderConnections fires cheap GET /v1/models requests
// at every configured provider so the HTTP transport's idle
// pool has warm TLS+TCP connections ready when the first real
// prompt lands. Runs in the background, never blocks startup.
// Best-case win: the first prompt skips 100-300ms of handshake
// for each new provider.
func (app *App) warmupProviderConnections(ctx context.Context) {
	providers := app.config.Config().Providers
	if providers == nil {
		return
	}
	for _, p := range providers.Seq2() {
		if p.BaseURL == "" || p.APIKey == "" {
			continue
		}
		// Only openai-compat providers expose a /v1/models endpoint
		// that matches the rest of their request shape.
		if p.Type != catwalk.TypeOpenAICompat {
			continue
		}
		base := strings.TrimRight(p.BaseURL, "/")
		// Derive a model-listing URL. Most openai-compat servers
		// expose /v1/models, so strip a trailing /v1 if present
		// and re-append.
		modelsURL := base
		if strings.HasSuffix(modelsURL, "/v1") {
			modelsURL = strings.TrimSuffix(modelsURL, "/v1") + "/v1/models"
		} else {
			modelsURL += "/v1/models"
		}
		go func(name, url, key string) {
			// Short, bounded request. We don't care about the
			// response body, only that the connection lands in
...
func (app *App) Shutdown() {
	...
	// Wait for any in-flight speculative read-ahead goroutines
	// to finish. Bounded by the shutdown context so we don't
	// hang on shutdown if a speculative view is wedged.
	if app.AgentCoordinator != nil {
		if s := app.AgentCoordinator.Speculator(); s != nil {
