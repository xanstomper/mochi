"""
Zhipu Bridge — GLM 5, GLM 5.1, GLM 5.2
...
GLM_MODELS: Dict[str, Dict[str, Any]] = {
    "glm-5": {
        "description": "GLM-5 — base model, general purpose",
        "max_tokens": 131072,
        "max_output": 8192,
    ...
    "glm-5.1": {
        "description": "GLM-5.1 — enhanced reasoning and knowledge",
        "max_tokens": 131072,
        "max_output": 16384,
        "pricing": (0.0005, 0.0020),
        "supports_streaming": True,
    ...
    "glm-5.2": {
        "description": "GLM-5.2 — frontier multimodal model",
        "max_tokens": 262144,
        "max_output": 32768,
        "pricing": (0.0008, 0.0030),
        "supports_streaming": True,
        "supports_tools": True,
        "supports_vision": True,
        "family": "glm-5",
...
class GLMStreamHandler:
    """Parses Zhipu GLM SSE stream chunks (OpenAI-compatible)."""
...
class ZhipuBridge(BaseProviderBridge):
    """Bridge for Zhipu GLM API (OpenAI-compatible).

    Supports GLM 5, GLM 5.1, and GLM 5.2 models.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        api_key = self._config.api_key or os.environ.get("ZHIPU_API_KEY", "")
        self._config.api_key = api_key

        # Default base_url
        if not self._config.base_url:
            self._config.base_url = "https://open.bigmodel.cn/api/paas/v4"

        # Default model
        if not self._config.default_model:
            self._config.default_model = "glm-5"
    ...
    def generate_stream(
        ...
        **kwargs: Any,
        ...
        headers = self._build_headers()
        client = self._get_client()
        handler = GLMStreamHandler(effective_model)
        ...
        try:
            ...
            yield BridgeStreamEvent(
                ...
                error=f"GLM stream timed out: {e}",
                model=effective_model,
            )
            self._total_errors += 1
            return
            ...
            log.exception("GLM stream error")
            yield BridgeStreamEvent(
                event_type=BridgeStreamEventType.ERROR,
                error=str(e),
                model=effective_model,
    ...
    def get_model_info(self, model: str) -> Optional[Dict[str, Any]]:
        for model_id, info in GLM_MODELS.items():
