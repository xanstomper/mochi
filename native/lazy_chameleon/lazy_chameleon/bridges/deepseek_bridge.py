...
DEEPSEEK_MODELS: Dict[str, Dict[str, Any]] = {
    ...
    "deepseek-v3": {
        ...
        "max_tokens": 128000,
        "max_output": 8192,
        "pricing": (0.00027, 0.00110),
        "supports_streaming": True,
        "supports_reasoning": False,
        "family": "v3",
    },
    "deepseek-v2": {
        "description": "DeepSeek-V2 — MoE architecture",
        "max_tokens": 128000,
        "max_output": 8192,
        "pricing": (0.00022, 0.00076),
        "supports_streaming": True,
        "supports_reasoning": False,
        "family": "v2",
    },
    "deepseek-chat": {
        "description": "DeepSeek-Chat — general chat model",
        "max_tokens": 128000,
        "max_output": 8192,
        "pricing": (0.00027, 0.00110),
        "supports_streaming": True,
...
# ═══════════════════════════════════════════════════════════════════════════════
# DeepSeek Stream Handler
# ═══════════════════════════════════════════════════════════════════════════════


class DeepSeekStreamHandler:
    """Parses DeepSeek SSE stream chunks (OpenAI-compatible)."""
...
class DeepSeekBridge(BaseProviderBridge):
    """Bridge for DeepSeek API (OpenAI-compatible).

    Supports DeepSeek-R1, V3, V2, Chat, and Coder models.
    """
    ...
    def __init__(
        ...
        **kwargs: Any,
        ...
        api_key = self._config.api_key or os.environ.get("DEEPSEEK_API_KEY", "")
        self._config.api_key = api_key

        # Default base_url
        if not self._config.base_url:
            self._config.base_url = "https://api.deepseek.com/v1"

        # Default model
        if not self._config.default_model:
            self._config.default_model = "deepseek-chat"
    ...
    def _build_request_body(
        ...
        **kwargs: Any,
        ...
        # R1-specific: enable reasoning mode
        if "r1" in model.lower():
            body.setdefault("reasoning_mode", True)
    ...
    def generate_stream(
        ...
        **kwargs: Any,
        ...
        headers = self._build_headers()
        client = self._get_client()
        handler = DeepSeekStreamHandler(effective_model)
        ...
        try:
            ...
            yield BridgeStreamEvent(
                ...
                error=f"DeepSeek stream timed out: {e}",
                model=effective_model,
            )
            self._total_errors += 1
            return
            ...
            log.exception("DeepSeek stream error")
            yield BridgeStreamEvent(
                event_type=BridgeStreamEventType.ERROR,
                error=str(e),
                model=effective_model,
